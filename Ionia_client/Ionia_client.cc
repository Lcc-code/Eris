...
#define USE_HASH_AGTR_INDEX 0
static int MAX_SEND_NUM = 20000;
static int MAX_KEY_NUM = 6000;
static const int batch_packet_num =48; // boardcast 50 * 2
...
{   
    ...
    uint32_t bitmap_1 = htonl(0x1);
    uint32_t bitmap_2 = htonl(0x2);
    uint32_t bitmap_3 = htonl(0x4);
    ...
    while (1) {
        ...
        if (send_position + batch_packet_num > (my_send_queue_length))
            send_position = 0;
        while (send_batch_num < batch_packet_num) {
            if (requests_[send_num].opcode == 1)
                ...
                dst_IP = read_dst_IP;
                        ...
                        // multicast
            if (requests_[send_num].opcode == 1) {
                make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num) * LAYER_SIZE, &bitmap_1, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log, 0, 0, &dst_IP);
                make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num + 1) * LAYER_SIZE, &bitmap_2, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log, 0, 0, &dst_IP);    
                make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num + 2) * LAYER_SIZE, &bitmap_3, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log, 0, 0, &dst_IP);        
                send_batch_num = send_batch_num + 3;
            } else {
                if (random == 0) {
                    make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num) * LAYER_SIZE, &bitmap_1, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log, 0, 0, &dst_IP);
                    make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num + 1) * LAYER_SIZE, &bitmap_2, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log_1, 0, 0, &dst_IP);
                    random = random + 1;
                    send_batch_num = send_batch_num + 2;
                    req_position = req_position + 1;
                } else if (random == 1) {
                    make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num) * LAYER_SIZE, &bitmap_2, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log, 0, 0, &dst_IP);
                    random = random + 1;
                    send_batch_num = send_batch_num + 1;
                } else {
                    make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num) * LAYER_SIZE, &bitmap_3, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log, 0, 0, &dst_IP);
                    make_aggr_layer_and_copy_to(dma->send_region + (send_position + send_batch_num + 1) * LAYER_SIZE, &bitmap_2, &num_worker, &version, requests_[send_num].opcode,
                                            &requests_[send_num].key, &requests_[send_num].index, &send_num, &num_log_1, 0, 0, &dst_IP);
                    random = 0;
        ...
        send_packet(dma, (LAYER_SIZE) * send_batch_num, send_position, thread_id, isServer);
        send_now = std::chrono::high_resolution_clock::now();
        send_position = send_position + send_batch_num;
        if (send_position + send_batch_num > my_send_queue_length)
            send_position = 0;
        ...
        while (1){
            ...
            for (int i = 0; i < msgs_completed; i++)
            {
                ...
                if (recve_header->num_log == 2)
                {
                    // p4ml_header_print(recve_header, "NUM2");
                } else {
                    uint32_t seq = ntohl(recve_header->seq);
                    uint8_t  flag = recve_header->flags;
                    uint32_t bitmap = ntohl(recve_header->bitmap);
                    
                    if (requests_[seq].ack == 0 || requests_[seq].ack == 1 || requests_[seq].ack == 2) {
                        // printf("seq %u requests_[seq].bitmap %lu bitmap %lu\n", seq, requests_[seq].bitmap, bitmap);
                            if (requests_[seq].bitmap != requests_[seq].bitmap | bitmap) // not receive
                            {
                                requests_[seq].bitmap = requests_[seq].bitmap | bitmap; // update bitmap
                                requests_[seq].ack = requests_[seq].ack + 1;
                                if ((recve_header->opcode == 1 && requests_[seq].ack == 2) || (recve_header->opcode == 0 && requests_[seq].ack == 1)) {
                                    if (seq % SAMPLE == 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            // std::cout << "2 seq / SAMPLE " << seq << " "<< SAMPLE << " " << std::endl;
                                        }
                                    }
                                }
                                ...
                                if (DEBUG_PRINT_ALL_DUP_PACKET)
                                    p4ml_header_print(recve_header, "DULP bitmap");
                            }                       
                    }  else {
                        recv_totoal_num = recv_totoal_num - 1;
                        if (DEBUG_PRINT_ALL_DUP_PACKET)
            ...
            // printf("recv_totoal_num %d msgs_completed %d send_batch_num %d\n", recv_totoal_num, msgs_completed, send_batch_num);
            dma_update_snapshot(dma, cur_snapshot);
            msgs_completed = 0;
            

            if (recv_totoal_num >= send_batch_num)
            {
                recv_totoal_num = 0;
                send_batch_num = 0;
                req_position = 0;
...
int main(int argc, char **argv)
{
    ...
    {
        hash_table[i] = new HashTable(UsedSwitchAGTRcount);
        
        for (int j = 0; j < UsedSwitchAGTRcount; j++)
        {
            if (USE_HASH_AGTR_INDEX)
                hash_table[i]->HashNew_crc(i, j);
            else 
                hash_table[i]->HashNew_linear(j);
            // printf("%d %d\n", j, hash_table[i]->hash_map[j]);
        }
        hash_table[i]->Clear_HashTable(UsedSwitchAGTRcount);
    }
    // return -1;

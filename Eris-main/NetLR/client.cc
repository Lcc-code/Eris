...
{   
    ...
    auto send_timeout = std::chrono::high_resolution_clock::now();
    while (1) {
        ...
        while (1){
            while (1)
            {
                ...
                // else {
                //     auto send_timeout_1 = std::chrono::high_resolution_clock::now();
                //     auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(send_timeout_1 - send_timeout);
                //     if (double(duration.count() / 1000/1000) > 100000000000) // Timeout 20ms
                //     {
                //         send_timeout = send_timeout_1;
                //         int need_resend = 0;
                //         if (send_position + batch_packet_num > (my_send_queue_length))
                //             send_position = 0;
                //         for (uint16_t i = send_num; i > send_num - batch_packet_num; i--)
                //         {
                //             if (requests_[i].ack == 0)
                //             {
                //                 make_aggr_layer_and_copy_to(dma->send_region + (send_position + need_resend) * LAYER_SIZE, &bitmap, &num_worker, &version, requests_[i].opcode,
                //                             &requests_[i].key, &requests_[i].index, &i, &num_log, 1, 0, &dst_IP);
                //                 need_resend = need_resend + 1;
                //             }
                //         }
                //         if (need_resend !=0){
                //             send_packet(dma, (LAYER_SIZE) * need_resend, send_position, thread_id, isServer);
                //             // printf("resend :%d\n", need_resend);
                            
                //             send_position = send_position + need_resend;
                //             if (send_position + need_resend > my_send_queue_length)
                //                 send_position = 0;
                //         }
            ...
            for (int i = 0; i < msgs_completed; i++)
            {
                ...
                if (flag == 8) { // collision
                    ...
                    uint16_t new_index = hash_table[thread_id]->predefine_agtr_list[seq % UsedSwitchAGTRcount];
    ...
    for (int i = 0; i < MAX_KEY_NUM; i++)
    {
        if (write_receive_time[i] != T_now && write_send_time[i] != T_now)
        {
            ...
            if (double(duration_write.count()) != 0)
            {
                ...
                std::cout << i << " write " << duration_write.count() << "ns " << std::endl;

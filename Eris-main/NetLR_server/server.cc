...
// #define ENABLE_ROCKSDB false
// #define ENABLE_VERSION_CHECK false
#define ENABLE_ROCKSDB true
#define ENABLE_VERSION_CHECK true

std::vector<std::atomic<int>> VERSION_VEC(100000); // 
...
void sync_wirte_main_loop(DMAcontext* dma, Log* log, int thread_id, bool isWrite)
{
    int cpu = 0;
    if (mlx_i == 0)
        cpu = thread_id;
    ...
    bindingCPU(cpu);
    printf("sync_wirte_main_loop :%d cpu %d\n", thread_id, cpu);
        ...
        while (1)
        {
            ...
            for (int i = 0; i < msgs_completed; i++)
            {
                ...
                if (ether_header->h_proto == N_OPTKVPROTO){ // 0x0700 = 
                    ...
                    log->append_write((&agg_header->key), agg_header->opcode, agg_header->version, value_len, (agg_header->flags), (&agg_header->seq), (&agg_header->index), (agg_header->num_log)); // ntohl(data) in append
        ...
        while (log_num > 0)
        {
            ...
            if (log_num >= batch_num)
            {
                ...
                for (int i = 0; i < batch_num; i++)
                {
                    ...
                    uint32_t cur_version = VERSION_VEC[key].load(std::memory_order_relaxed);
                    if (ENABLE_VERSION_CHECK){
                        if (ENABLE_ROCKSDB)
                        {
                            if (cur_version < version) {// if write version is higher than cur version
                                VERSION_VEC[key].store(version, std::memory_order_release);
...
int main(int argc, char **argv)
{
    ...
    std::fill(VERSION_VEC.begin(), VERSION_VEC.end(), 0);
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        mainLoopThreadQueue[i+1] = new std::thread(sync_wirte_main_loop, dmaContextQueue[i+1], LogQueue[i], i, isWrite); // write

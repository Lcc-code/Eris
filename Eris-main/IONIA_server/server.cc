...
void inline DB_READ(std::string &key, std::string &value)
{
    auto stats = db->Get(ReadOptions(), key, &value);
    std::cout << "value" << value << std::endl;
    assert(stats.ok());
    // assert(value == "value");
}
    ...
    while (1)
    {
        ...
        while (log_num > 0)
        {
            ...
            if (log_num >= batch_num)
            {
                ...
                if (send_position + batch_num >= (my_send_queue_length / 2))
                    send_position = 0;
        
                for (int i = 0; i < batch_num; i++)
                {
                    LogEntry *LE = log->consume(isWrite);
                    if (LE->num_log == 1) // if 1 read, else not read
                    {
                        uint32_t key = ntohl(LE->key);
                        uint32_t version = ntohl(LE->version);
                        uint32_t cur_version = VERSION_VEC[key].load(std::memory_order_relaxed);
